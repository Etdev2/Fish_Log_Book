/**
 * Regenerate supabase/migrations/20260902160000_v1_pnw_wa_or_packs.sql — Pacific
 * Northwest wave: Oregon + Washington v1 packs (ODFW/WDFW/NOAA verbatims) plus the
 * species rows the packs reference. Insert-only; gs and areas are new ids so nothing
 * existing is touched. Alaska ships a Settings region with NO pack on purpose.
 *
 *   npx tsx scripts/gen-pnw-wave.mts
 */
...
const NEW_SPECIES = [
  "canary_rockfish", "blue_rockfish", "black_rockfish", "yellowtail_rockfish",
  "widow_rockfish", "bocaccio", "copper_rockfish", "quillback_rockfish",
  "vermilion_rockfish", "yelloweye_rockfish", "sablefish", "pacific_cod",
  "shiner_perch", "sixgill_shark", "flatfish",
];

let sql = `-- Pacific Northwest wave v1 (24-state expansion, 2026-09-02): Oregon (ODFW) and
-- Washington (WDFW) ocean packs. GENERATED from the bundles via gen-pnw-wave.mts.
...
const emit = (bundle: typeof OREGON) => {
...
writeFileSync("supabase/migrations/20260902160000_v1_pnw_wa_or_packs.sql", sql);
console.log(`written ${sql.length} bytes; ${NEW_SPECIES.length} species, ${OREGON.rules.length} OR + ${WASHINGTON.rules.length} WA rules`);
