...
| A2 | **Photo with EXIF stripped** | Already in the spec as V2. The privacy half matters: photos carry GPS, and anglers do not share spots. **✅ REVERSED — 2026-09-15, founder. See SPEC.md D28.** Was "❌ NOT NOW — 2026-09-03, founder, on cost": no media table, no storage, no EXIF stripping, no moderation budget, and verification levels 1–2 struck from Phase 2 of the passport spec (its §45.2). Photos are back in scope, and EXIF stripping on ingest is part of the deal rather than a later refinement. This unblocks AI fish ID, fish biometrics and tournament photo evidence — see `docs/specs/expansion/`. The cost that motivated the original no has not gone away; `cfo` prices it before bulk upload ships. | S |
